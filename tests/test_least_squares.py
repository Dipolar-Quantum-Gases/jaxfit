...
class ExponentialFittingProblem:
    ...
    def fun(self, p):
        return p[0] + jnp.exp(p[1] * self.x) - self.y
...
class BoundsMixin:
    ...
    def test_bounds_shape(self):
        for jac in [None, jac_2d_trivial]:
            x0 = [1.0, 1.0]
...
class LossFunctionMixin:
    ...
    def test_fun(self):
        ...
        for loss in LOSSES:
            res = least_squares(fun_trivial, 2.0, loss=loss,
                                method=self.method)
            assert_equal(np.array(res.fun), fun_trivial(res.x))
    ...
    def test_jac(self):
        ...
        res = least_squares(fun_trivial, x, jac_trivial, loss='linear',
                            max_nfev=1, method=self.method)
        assert_equal(np.array(res.jac)[0,0], 2 * x)
